...
行情功能插件：把 DS 指数与代码量 K 线做成真实可用的界面组件。它占用两个上游扩展缝 —— `conversation.bottom.panel`（代码量日 K 线停靠）与 `conversation.details.panel`（右侧 DS 指数炒股界面）。所有数字来自运行时会话的确定性派生投影（`src/client/market-data.ts`）：节点数、turn 计时与会话列表是真实 DSH 数据，指数值由它们映射，并统一标注 LOCAL PROJECTION；本插件不引入任何证券数据供应商，也不伪装实时行情。
